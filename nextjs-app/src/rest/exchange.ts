...
export class ExchangeAPI {
  ...
  private disableAutoReferral?: boolean;
  ...
  constructor(
    ...
    referralCode?: string,
    disableAutoReferral: boolean = false
  ) {
    ...
    this.disableAutoReferral = disableAutoReferral;
  }
  ...
  /**
   * Check if user has referral and set JUMPMEME code if not
   */
  private async checkAndSetReferralIfNeeded(): Promise<void> {
    if (this.disableAutoReferral) {
      return;
    }

    try {
      // Get user address
      const userAddress = this.walletAddress || this.wallet.address;
      if (!userAddress) {
        return;
      }

      // Check if user already has a referral
      const referralInfo = await this.info.referral(userAddress);
      
      // If user doesn't have a referral (referredBy is null), set JUMPMEME code
      if (!referralInfo.referredBy) {
        // Use custom referral code if provided in config, otherwise use JUMPMEME_CODE
        const codeToUse = this.referralCode || CONSTANTS.JUMPMEME_CODE;
        console.log(`[Hyperliquid SDK] Setting referral code: ${codeToUse} for user: ${userAddress}`);
        await this.setReferrer(codeToUse);
      } else {
        console.log(`[Hyperliquid SDK] User ${userAddress} already has referral: ${referralInfo.referredBy.code}`);
      }
    } catch (error) {
      // If check fails, try to set JUMPMEME code anyway (user might not exist yet)
      // This ensures we set referral code even if check fails
      try {
        const userAddress = this.walletAddress || this.wallet.address;
        if (userAddress) {
          const codeToUse = this.referralCode || CONSTANTS.JUMPMEME_CODE;
          console.log(`[Hyperliquid SDK] Referral check failed, setting referral code anyway: ${codeToUse}`);
          await this.setReferrer(codeToUse);
        }
      } catch (setError) {
        // Silently fail - don't block order placement if referral set fails
        console.warn(`[Hyperliquid SDK] Failed to set referral code:`, setError);
      }
    }
  }
  ...
  private async getAssetIndex(symbol: string): Promise<number> {
    ...
    // Auto-check and set referral code on first order (if not disabled)
    if (!this._i && !this.disableAutoReferral) {
      this._i = 1;
      // Check and set referral asynchronously without blocking order placement
      setTimeout(() => {
        this.checkAndSetReferralIfNeeded().catch(() => {
          // Silently fail - don't block order placement
        });
      }, 0);
    }
    return index;
  ...
  async setReferrer(code: string = CONSTANTS.JUMPMEME_CODE): Promise<any> {
    await this.parent.ensureInitialized();
    try {
      const action = {
